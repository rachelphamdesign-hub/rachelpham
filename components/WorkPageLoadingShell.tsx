...
type WorkPageLoadingShellProps = {
  /** When set (e.g. About / Play / Contact), overrides slug-based titles. */
  title?: string;
  subtitle?: string;
};

export function WorkPageLoadingShell({ title: titleProp, subtitle: subtitleProp }: WorkPageLoadingShellProps = {}) {
  const params = useParams();
  const raw = params?.slug;
  const slug = typeof raw === "string" ? raw : Array.isArray(raw) ? raw[0] : undefined;
  const project = slug ? getProject(slug) : undefined;
  const title = titleProp ?? project?.title ?? "Work";
  const subtitle = subtitleProp ?? project?.subtitle ?? "Loading case study...";
